...
description: Research all required product, auth, metadata, API, and implementation details for an OvalEdge connector with zero assumptions. Use when user asks to research a connector before implementation.
---

# Connector Research Skill (No-Assumption Mode)

## When to use

- Before **build-new-connector-sdk** or before filling the csp-api **Create New Connector** form.
- When auth, object model, or API behavior is unclear — do not start coding until critical gaps are resolved or marked `Unconfirmed`.

## Deliverable

- Write findings to `{connector-id}/Research.md` (or user-specified path), using the sections below.
- End with the **Summary Table** and an explicit **Open questions** list for anything still `Unconfirmed` or `Not found yet`.

## Next step

- After research is approved, use **build-new-connector-sdk** (or scaffold via `.docs/sdk/06.Generate_the_new_Connector_Source.md` then implement).

## Mandatory behavior (strict)

- **Ask for missing inputs first**: If connector name, exact product variant/edition, deployment model (cloud/on-prem), or target APIs are unclear, ask the user before proceeding.
- **Strictly no assumptions**: Never assume vendor endpoints, auth flows, object hierarchy, pagination style, rate limits, or field mappings. If not explicitly confirmed by official docs or user input, treat as unknown and ask.
- **Evidence-first output**: Mark each finding as either:
  - `Confirmed` (official documentation / user-provided evidence), or
  - `Unconfirmed` (needs user decision or additional evidence).
- **Clarify ambiguity immediately**: If a product name can map to multiple products (e.g., editions, similarly named tools), pause and ask.
- **Do not invent data**: For unavailable details, explicitly state `Not found yet` and ask follow-up questions.
...
- OvalEdge contracts: `.docs/Connector_Interface_Reference.md` (`ObjectKind`, metadata/query lifecycle).
- Example research artifact: `zohodesk/Research.md`.
