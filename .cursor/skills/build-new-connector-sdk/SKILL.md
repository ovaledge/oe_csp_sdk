---
name: build-new-connector-sdk
description: Build a new Apps connector module in oe_csp_sdk with strict no-assumption behavior and mandatory clarifications when inputs are missing or ambiguous.
---

# Build New Connector SDK Skill (No-Assumption Mode)

## When to use

- Implementing connector logic after **connector-research** (or equivalent user-provided spec).
- Extending a module already scaffolded via Maven archetype or csp-api **Create New Connector** (see `.docs/sdk/06.Generate_the_new_Connector_Source.md`).

## Related skills (workflow order)

1. **connector-research** — gather Confirmed/Unconfirmed spec first (strict, no assumptions).
2. **build-new-connector-sdk** (this skill) — implement module.
3. **unit-test-generation** — expand archetype test TODOs.
4. **connector-code-review** — pre-PR checklist vs Definition of Done.
5. **connector-debugging** — if build, SPI, or runtime issues appear.
6. **implement-process-app-objects-for-edgi** — only when EDGI bridge is required.

**Contract source of truth:** `.docs/Connector_Interface_Reference.md` and `.docs/sdk/10.Definition_of_Done_and_Success_Criteria.md`.

## Mandatory behavior (strict)

- **Ask before generating**: If connector name, server type, data source type, authentication details, endpoints, object kinds, or required attributes are missing/ambiguous, ask the user first.
- **Strictly no assumptions**: Do not infer URLs, auth flows, object models, field names, pagination, or constants.
- **Clarify ambiguity**: If one name can mean multiple products/variants, pause and confirm.
- **Use provided details only**: If details are unavailable, request them explicitly and do not fabricate placeholders as facts.
- **Fail-safe behavior**: If a required decision is not provided, stop generation and ask targeted questions.
...
- **Connector module pom.xml:** Parent `oe-csp-sdk`, artifactId `{connector-id}`, dependencies: `csp-sdk-core`, plus any connector-specific libs (e.g. `spring-boot-starter-web`, `org.json`, `httpclient5`, logging). No need to duplicate versions if in parent dependencyManagement.

---

## 8. Assets and registration config

- **Icon:** `src/main/resources/static/{connector-id}.png` (or `{server-type}.png`). Used in UI.
- **Registration JSON:** `src/main/resources/configs/{connector-id}.json` — set `connectorMaster.server` to `{SERVER_TYPE}` and `dtoRegisterName` to the connector FQCN. Do not invent capability flags; align with user-confirmed crawler/query options from research.
...
- [ ] **`@SdkConnector(artifactId = "{connector-id}")`** on connector class; `configs/{connector-id}.json` present with matching `server` / `dtoRegisterName`.
...
- [ ] Connector appears in `GET http://localhost:8800/v1/info` after `csp-api` rebuild (see **connector-debugging** if missing).
...
- [ ] If EDGI is required: `processAppObjectsForEdgi` implemented (use **implement-process-app-objects-for-edgi** skill).
- [ ] Run **connector-code-review** before PR; public catalog also requires tests per **unit-test-generation** and `.docs/sdk/10.Definition_of_Done_and_Success_Criteria.md`.

---

## Usage

Replace `{CONNECTOR_NAME}`, `{SERVER_TYPE}`, and connector-id/package/prefix with the actual values. Use the **monetdb** module as the reference for JDBC/DBMS connectors; use **zohodesk** for REST/OAuth2. Keep this skill scoped to oe_csp_sdk only (no csp-lib/DbDto/DB crawl logic). Package layout: `com.ovaledge.csp.apps.{package}` where `{package}` is `{connector-id}` with hyphens removed.
