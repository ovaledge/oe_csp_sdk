...
| **Legacy/duplicate serverType guard** | Connector names may only contain letters, digits, spaces, and hyphens (`-`). Normalization lowercases, strips other characters, collapses spaces to hyphens (e.g. `Qlik Sense` → `qlik-sense`). Legacy reserved ids from `csp-api/src/main/resources/legacy-platform-server-types.txt` are compared in normalized form; package-level conflicts use the compact id (hyphens removed), e.g. `Monet DB` / `monet-db` → `monetdb`. In-repo SDK connector `serverType` values are discovered dynamically from reactor modules (union of `configs/*.json` basenames and `getServerType()`). **New connectors are blocked** when the normalized name is on the legacy txt list **or** already owned in the repo. `GET /v1/generator/reserved-server-types`, `GET /v1/generator/normalize`, and the Connector Generator UI enforce that policy (live input sanitization + server validation); the UI is fail-closed until reserved types load. Assembly build (`LegacyServerTypeForbiddenTest` via `SdkConnectorReactorScanner`) enforces legacy/duplicate rules, duplicate provider FQCNs, config basename vs `getServerType()` consistency, and strict reflection (no-arg constructor). Unlisted connector directories (SPI present but not in root `pom.xml` modules) log a warning only. |
