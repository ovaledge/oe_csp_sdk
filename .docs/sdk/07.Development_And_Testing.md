# 7. Connector Development & Testing

Refer to [Connector Interface Reference](../Connector_Interface_Reference.md) to implement the services required for connector

---

## 7.1. Compile and Build

From the **repository root:**

| Goal | Command | Result |
|---|---|---|
| Build one connector | `mvn clean install -pl amazonq -am` | Builds `amazonq` and its dependencies. |
| Build everything | `mvn clean install` | Builds all modules (connectors, csp-api, assembly). |
| Build assembly JAR only | `mvn clean install -pl assembly -am` | Builds all modules and produces `assembly/target/csp-sdk-*.jar`. |

---

## 7.2. Run and test

### Option A – Unified API (all connectors)

1. Build: `mvn clean install -pl csp-api -am` (or `mvn clean install`).
2. Run: `java -jar csp-api/target/csp-api-*.jar` — default port is **8800** (see `csp-api/src/main/resources/application.properties`).
3. Open the built-in UI at [http://localhost:8800/](http://localhost:8800/) to select a connector, set connection attributes, and call connection validate, metadata, and query.
4. Call REST endpoints with `serverType` in the request body (e.g., `"serverType": "monetdb"`).
    - `GET /v1/info` does **not** require a request body.
    - Metadata / query / validate endpoints require a request body and (for some endpoints) query parameters.

Example:

```bash
curl -s http://localhost:8800/v1/info
```

> **Note:** If you changed the default port in `application.properties`, update the `curl` URL accordingly.

### Option B – Standalone connector (quick app)

For a connector that has a `quick` package (e.g., `monetdb`):

```bash
mvn -pl monetdb spring-boot:run
```

Or run the JAR of that module (main class is configured in its `pom.xml`).

---

## 7.3. Code Review Standards and Quality Gates

For a consolidated checklist, see [10. Definition of Done and Success Criteria](10.Definition_of_Done_and_Success_Criteria.md).

All PRs — regardless of developer persona — must pass the following gates before merge:

### 7.3.1 Automated Checks (GitHub Actions)

The following checks run automatically on every PR:

| Check | Description |
|---|---|
| **Build verification** | `mvn clean install` passes across all modules. |
| **Unit test pass** | All connector unit tests pass with no failures. |
| **SAST scan** | Static Application Security Testing via the configured scanner (e.g., CodeQL or Semgrep). Zero high-severity findings. |
| **Dependency vulnerability scan** | OWASP Dependency-Check or equivalent. No known critical CVEs in connector dependencies. |
| **Connector contract validation** | Automated check that `AppsConnector`, `MetadataService`, and `QueryService` required methods are present. `getProfilingService()` is optional (default `null`); when overridden, JSON capability flags must match. |
| **Legacy/duplicate serverType guard** | Connector names may only contain lowercase letters and digits, starting with a letter (e.g. `qliksense`, `mysql2`). Normalization lowercases and strips all other characters for legacy comparison (e.g. `Qlik Sense` → `qliksense`). Legacy reserved ids from `csp-api/src/main/resources/legacy-platform-server-types.txt` are compared in normalized form. In-repo SDK connector `serverType` values are discovered dynamically from reactor modules (union of `configs/*.json` basenames and `getServerType()`). **New connectors are blocked** when the normalized name is on the legacy txt list **or** already owned in the repo. Versioned alternates use a `vN` suffix (e.g. `mysqlv2`). `GET /v1/generator/reserved-server-types`, `GET /v1/generator/normalize`, and the Connector Generator UI enforce that policy (live input sanitization + server validation); the UI is fail-closed until reserved types load. Assembly build (`CspSdkServerTypeValidator` Mojo via `SdkConnectorReactorScanner`, run during `mvn -pl assembly package`) enforces legacy/duplicate rules, duplicate provider FQCNs, config basename vs `getServerType()` consistency, and strict reflection (no-arg constructor). Unlisted connector directories (SPI present but not in root `pom.xml` modules) log a warning only. |
| **SPI registration check** | `META-INF/services/` file is present and references a valid FQCN. |
| **No hardcoded secrets** | Secrets detection scan (e.g., git-secrets or truffleHog). PR fails if credentials, tokens, or API keys are detected in code. |

PRs with failing automated checks will **not proceed to manual review** until all issues are resolved.

### 7.3.2 Manual Review Criteria

OvalEdge reviewers assess the following:

| Criterion | Standard |
|---|---|
| **Connector contract compliance** | Required interface methods implemented correctly per the [Connector Interface Reference](../Connector_Interface_Reference.md). Optional `getProfilingService()` is non-null only when JSON declares profiling. No silent exception swallowing. |
| **Error handling** | Errors surfaced with meaningful messages per the conventions in Tab 1, Section 5.4. |
| **Logging hygiene** | No credentials, tokens, or PII logged at any level. Logging follows Tab 1 Section 5.5 conventions. |
| **Security practices** | No hardcoded credentials. Input validation on all connection config parameters. Safe handling of external data. Follows OWASP Top 10 guidelines. |
| **Code quality** | Readable, maintainable Java. No unnecessary complexity. Classes and methods are appropriately sized. |
| **Test coverage** | Unit tests cover happy path, error scenarios, and edge cases for all public methods. |
| **Documentation** | `README.md` or `INSTRUCTIONS.txt` present. End-user connector documentation completed per the [Connector Documentation Template](../templates/OvalEdge_Connector_Documentation_Template.md). `@SdkConnector` annotation populated. Release properties file included. |

### 7.3.3 Code Review SLA

| Developer Persona | Review SLA |
|---|---|
| Partners (public catalog submission) | 10 business days from PR submission |
| Open Source Contributors | 15 business days from PR submission |

If a review will exceed the SLA due to backlog, the OvalEdge reviewer will post a comment on the PR with an updated estimated timeline.

### 7.3.4 Testing Requirements Before Raising a PR

Before raising a PR, developers must confirm:

- [ ] `mvn clean install` passes locally with no errors.
- [ ] The connector appears in `GET /v1/info` when running `csp-api` locally.
- [ ] `validateConnection()` returns a successful response with valid test credentials.
- [ ] `getSupportedObjects()`, `getContainers()`, `getObjects()`, and `getFields()` return correct results.
- [ ] `fetchData()` returns correct results for at least one supported entity type.
- [ ] No credentials, tokens, or PII are present in code or logs.
- [ ] The PR description includes: connector name, target data source, test results (paste console output, a log snippet, or a screenshot — any format is acceptable), and any known limitations.

---

## 7.4. Additional Information

As OvalEdge publishes updates to the master public SDK, each fork must stay current by syncing **`upstream/main`** (public repo) into **`origin/main`** (your fork’s `main` branch). Developers are responsible for syncing before beginning new work.

### 7.4.1 Configuring Upstream Remote

```bash
git remote add upstream https://github.com/ovaledge/oe_csp_sdk.git
git remote -v   # Verify both origin and upstream are listed
```

### 7.4.2 Syncing Changes

Sync from the public repository’s `main` branch to your fork’s `main` branch:

```bash
git fetch upstream
git checkout main
git rebase upstream/main
git push origin main   # Use --force-with-lease if you already pushed main before rebasing
```

Resolve any merge conflicts before continuing development. If you use feature branches, rebase or merge them onto your updated fork `main` after this sync.

### 7.4.3 Breaking Change Notifications

- OvalEdge will publish SDK release notes in the repository `CHANGELOG.md` and notify registered developers via **developer@ovaledge.com** at least **30 calendar days** before a major version release containing breaking changes.
- Major version changes may require connector updates. OvalEdge will provide a migration guide with each major release.
- Minor and patch releases are backward-compatible and do not require connector changes.

### 7.4.4 Version Compatibility

- Connectors must declare their target `csp-sdk-core` version in their `pom.xml`.
- A version compatibility matrix (SDK version ↔ OvalEdge platform version) will be published at `https://docs.ovaledge.com/connectors/sdk-compatibility`.
- Developers should verify compatibility before starting work on a new connector.

---

[⬅️ Previous: 6. Generate the new Connector Source](06.Generate_the_new_Connector_Source.md)

[➡️ Next: 8. Assemble the new connector](08.Assemble_the_new_connector.md)
